...
export default class MazeGenerator {
    ...
    tick() {
        ...
        return () => {
            ...
            //      the current cell
            const collectNegihbors = ([row, col]: [number, number]): [Cell, number][] => {
            ...
            const removeWalls = (numRow, numCol, direction, bob=false) => {
                if (bob) {
                    debugger;
                }
                ...
                switch (direction) {
                    case DIRECTIONS.NORTH: {
                        self.grid[numRow][numCol].borders[DIRECTIONS.NORTH] = false;
                        self.grid[numRow - 1][numCol].borders[DIRECTIONS.SOUTH] = false;
                        break;
                    }
                    case DIRECTIONS.EAST: {
                        self.grid[numRow][numCol].borders[DIRECTIONS.EAST] = false;
                        self.grid[numRow][numCol + 1].borders[DIRECTIONS.WEST] = false;
                        break;
                    }
                    case DIRECTIONS.SOUTH: {
                        self.grid[numRow][numCol].borders[DIRECTIONS.SOUTH] = false;
                        self.grid[numRow + 1][numCol].borders[DIRECTIONS.NORTH] = false;
                        break;
                    }
                    case DIRECTIONS.WEST: {
                        self.grid[numRow][numCol].borders[DIRECTIONS.WEST] = false;
                        self.grid[numRow][numCol - 1].borders[DIRECTIONS.EAST] = false;
                        break;
                    }
                    default:
                }
            ...
            if (validNeighbors.length !== 0) {
                ...
                // NORTH
                if (chosenRow > 0 && chosen.borders[DIRECTIONS.NORTH] && self.grid[chosenRow - 1][chosenCol].state === Cell.STATES.PASSED) {
                    dirs.push(DIRECTIONS.NORTH);
                }
                // EAST
                if (chosenCol < self.grid[0].length - 1 && chosen.borders[DIRECTIONS.EAST] && self.grid[chosenRow][chosenCol + 1].state === Cell.STATES.PASSED) {
                    dirs.push(DIRECTIONS.EAST);
                }
                // SOUTH
                if (chosenRow < self.grid.length - 1 && chosen.borders[DIRECTIONS.SOUTH] && self.grid[chosenRow + 1][chosenCol].state === Cell.STATES.PASSED) {
                    dirs.push(DIRECTIONS.SOUTH);
                }
                // WEST
                if (chosenCol > 0 && chosen.borders[DIRECTIONS.WEST] && self.grid[chosenRow][chosenCol - 1].state === Cell.STATES.PASSED) {
                    dirs.push(DIRECTIONS.WEST);
                }

                const connectDir = parseInt(randRange(0, dirs.length));
                removeWalls(chosen.row, chosen.col, dirs[connectDir], true);
            }
